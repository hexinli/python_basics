...
def main2():
    """
    简要介绍json模块。
    json.dump: 将Python对象写入文件对象，参数：obj, fp, 其他可选参数（如ensure_ascii, indent等）。
    json.load: 从文件对象读取JSON数据并转换为Python对象，参数：fp, 其他可选参数。
    举例：先创建一个字典，用json.dump写入文件，再用json.load读取回来。
    注意文件打开模式：写为'w'，读为'r'，并指定编码（如encoding='utf-8'）以处理中文。
    """
    # 原始Python数据
    original_data = {
        "订单号": "ORD20240101001",
